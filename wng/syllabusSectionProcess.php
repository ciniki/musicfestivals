...
function ciniki_musicfestivals_wng_syllabusSectionProcess(&$ciniki, $tnid, &$request, $section) {
    ...
    if( isset($s['section-id']) ) {
        ...
        if( !isset($rc['section']) ) {
            return array('stat'=>'404', 'err'=>array('code'=>'ciniki.musicfestivals.868', 'msg'=>'Unable to find requested section'));
        }
        $syllabus_section = $rc['section'];
    }
    ...
    foreach(['top', 'bottom'] AS $tp) {
        ...
        for($i = 2; $i <= 5; $i++) {
            if( isset($s["section-{$tp}-button-{$i}-page"]) 
                ...
                && isset($s["section-{$tp}-button-{$i}-text"]) && $s["section-{$tp}-button-{$i}-text"] != '' 
                ) {
                $buttons[$tp][] = [
                    'page' => isset($s["section-{$tp}-button-{$i}-page"]) ? $s["section-{$tp}-button-{$i}-page"] : 0,
                    'url' => isset($s["section-{$tp}-button-{$i}-url"]) ? $s["section-{$tp}-button-{$i}-url"] : '',
            ...
            if( isset($category['classes']) && count($category['classes']) > 0 ) {
                ...
                foreach($category['classes'] as $cid => $class) {
                    ...
                    if( ($festival['flags']&0x01) == 0x01 
                        ...
                        $category['classes'][$cid]['register'] = "<a class='button' href='{$request['ssl_domain_base_url']}/account/musicfestival/{$festival['permalink']}/registrations?add=yes&cl=" . $class['uuid'] . "'>Register</a>";
                    } else {
