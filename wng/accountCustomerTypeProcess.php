...
function ciniki_musicfestivals_wng_accountCustomerTypeProcess(&$ciniki, $tnid, &$request, $args) {
    ...
    if( $customer_type == 10 ) {
        $rsp['switch_block'] = array(
            ...
            'content' => "<br/>You are a parent/guardian registering " . strtolower($args['festival']['competitor-label-plural']) . ".<br/><br/><a class='button' href='{$args['base_url']}?changetype'>Change Registration Type</a>"
            );
    } elseif( $customer_type == 20 ) {
        $rsp['switch_block'] = array(
            'type' => 'text',
            'class' => 'limit-width limit-width-60 aligncenter',
            'content' => "<br/>You are a teacher registering " . strtolower($args['festival']['competitor-label-plural']) . ".<br/><br/><a class='button' href='{$args['base_url']}?changetype'>Change Registration Type</a>"
            );
