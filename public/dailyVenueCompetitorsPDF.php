...
// -------
//
function ciniki_musicfestivals_dailyVenueCompetitorsPDF($ciniki) {
    //
    ...
    $rc = ciniki_musicfestivals_checkAccess($ciniki, $args['tnid'], 'ciniki.musicfestivals.commentsPDF');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    // Run the template
    //
    ciniki_core_loadMethod($ciniki, 'ciniki', 'musicfestivals', 'templates', 'dailyVenueCompetitorsPDF');
    $rc = ciniki_musicfestivals_templates_dailyVenueCompetitorsPDF($ciniki, $args['tnid'], $args);
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
