...
// -------
//
function ciniki_musicfestivals_buildingUpdate(&$ciniki) {
    //
    ...
    $rc = ciniki_musicfestivals_checkAccess($ciniki, $args['tnid'], 'ciniki.musicfestivals.buildingUpdate');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.musicfestivals.1216', 'msg'=>'Unable to load building', 'err'=>$rc['err']));
    }
    if( !isset($rc['building']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.musicfestivals.1217', 'msg'=>'Unable to find requested building'));
    }
    ...
    if( isset($args['name']) ) {
        ...
        $strsql = "SELECT id, name, permalink "
            ...
            . "AND festival_id = '" . ciniki_core_dbQuote($ciniki, $building['festival_id']) . "' "
            . "AND id <> '" . ciniki_core_dbQuote($ciniki, $args['building_id']) . "' "
            . "";
        ...
        if( $rc['num_rows'] > 0 ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.musicfestivals.1218', 'msg'=>'You already have an building with this name, please choose another.'));
        }
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.musicfestivals.building', $args['building_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    if( isset($args['sequence']) ) {
        ...
        $rc = ciniki_core_sequencesUpdate($ciniki, $args['tnid'], 'ciniki.musicfestivals.building', 
            'festival_id', $building['festival_id'], $args['sequence'], $building['sequence']);
        if( $rc['stat'] != 'ok' ) {
            ciniki_core_dbTransactionRollback($ciniki, 'ciniki.musicfestivals');
            return $rc;
        }
    ...
    ciniki_core_hookExec($ciniki, $args['tnid'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.musicfestivals.building', 'object_id'=>$args['building_id']));
