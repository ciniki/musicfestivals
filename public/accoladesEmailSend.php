...
// Description
// ===========
// This method will send an email to the list of recipients for the category/subcategory
//
// Arguments
// ---------
// api_key:
...
// // Returns // -------
//
function ciniki_musicfestivals_accoladesEmailSend(&$ciniki) {
...
//        'winner_id'=>array('required'=>'no', 'blank'=>'yes', 'name'=>'Winner'),
    ...
    $rc = ciniki_musicfestivals_checkAccess($ciniki, $args['tnid'], 'ciniki.musicfestivals.accoladesEmailSend');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    $strsql = "SELECT winners.id, "
        ...
        . "accolades.criteria AS accolade_criteria, "
        . "accolades.donor_thankyou_info, "
        . "categories.awarded_email_subject, "
        . "categories.awarded_email_content, "
    ...
    $rc = ciniki_core_dbHashQueryArrayTree($ciniki, $strsql, 'ciniki.musicfestivals', array(
        array('container'=>'winners', 'fname'=>'id', 
            'fields'=>array('id', 'flags', 'registration_id', 'awarded_amount', 'discipline',
                'accolade_name', 'accolade_criteria', 'donor_thankyou_info', 
//                'awarded_email_subject', 'awarded_email_content',
    ...
    $sent_emails = null;
    if( !preg_match("/\{_(accolade|accoladename|accoladecriteria|accoladeamount|awardedamount|discipline|thankyou)_\}/", $args['message']) ) {
        $sent_emails = []; 
    }
    ...
    //
    ciniki_core_loadMethod($ciniki, 'ciniki', 'musicfestivals', 'private', 'accoladeEmailSend');
    foreach($winners as $winner) {
        $rc = ciniki_musicfestivals_accoladeEmailSend($ciniki, $args['tnid'], [
            'festival' => $festival,
            'winner' => $winner,
        ...
        if( is_array($sent_emails) && isset($rc['sent_emails']) && is_array($rc['sent_emails']) ) {
            foreach($rc['sent_emails'] as $sent_email) {
