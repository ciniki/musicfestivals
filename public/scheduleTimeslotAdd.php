...
function ciniki_musicfestivals_scheduleTimeslotAdd(&$ciniki) {
    ...
    $rc = ciniki_core_prepareArgs($ciniki, 'no', array(
        ...
        'warmup_time'=>array('required'=>'no', 'blank'=>'yes', 'type'=>'time', 'name'=>'Time'),
        'warmup_offset_seconds'=>array('required'=>'no', 'blank'=>'yes', 'name'=>'Warm-Up Offset'),
        'warmup_seconds'=>array('required'=>'no', 'blank'=>'yes', 'name'=>'Warm-Up Length'),
    ...
    //
    if( isset($args['linked_timeslot_id']) && $args['linked_timeslot_id'] > 0 
        && ($timeslot['flags']&0x04) == 0x04 // Time Linked Timeslot
        && (isset($args['slot_time']) || isset($args['slot_seconds'])) 
        ) {
        $linked_args = [];
        ...
        $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.musicfestivals.scheduletimeslot', $args['linked_timeslot_id'], $linked_args, 0x04);
        if( $rc['stat'] != 'ok' ) {
            ciniki_core_dbTransactionRollback($ciniki, 'ciniki.musicfestivals');
            return $rc;
        }
