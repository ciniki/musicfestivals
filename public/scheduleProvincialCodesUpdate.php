...
// ---------
// 
function ciniki_musicfestivals_scheduleProvincialCodesUpdate(&$ciniki) {
    ...
    foreach($classes as $cid => $class) {
        $update_args = array();
        if( isset($ciniki['request']['args']["provincials_code_{$cid}"]) ) {
            $update_args['provincials_code'] = $ciniki['request']['args']["provincials_code_{$cid}"];
        }
        if( count($update_args) ) {
            ciniki_core_loadMethod($ciniki, 'ciniki', 'core', 'private', 'objectUpdate');
            $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.musicfestivals.class', $cid, $update_args, 0x04);
            if( $rc['stat'] != 'ok' ) {
