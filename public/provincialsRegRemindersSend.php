...
// // Returns // -------
//
function ciniki_musicfestivals_provincialsRegRemindersSend(&$ciniki) {
    ...
    $rc = ciniki_musicfestivals_checkAccess($ciniki, $args['tnid'], 'ciniki.musicfestivals.provincialsRegRemindersSend');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( !isset($member['reg_status']) || $member['reg_status'] != 'open' ) {
        return array('stat'=>'warn', 'err'=>array('code'=>'ciniki.musicfestivals.1490', 'msg'=>'Recommendation submissions are closed for your festival'));
    }
    ...
    if( !isset($festival['provincials-email-register-live-subject']) || $festival['provincials-email-register-live-subject'] == '' 
        ...
        return array('stat'=>'warn', 'err'=>array('code'=>'ciniki.musicfestivals.1491', 'msg'=>'No template configured'));
    }
    ...
    $strsql = "SELECT entries.id, " 
        ...
        . "AND entries.status = 45 "; // Invite Sent entries
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.musicfestivals.1492', 'msg'=>'Unable to load entries', 'err'=>$rc['err']));
    }
    ...
    //
    ciniki_core_loadMethod($ciniki, 'ciniki', 'musicfestivals', 'private', 'provincialsRegReminderSend');
    foreach($entries as $entry) {
        $rc = ciniki_musicfestivals_provincialsRegReminderSend($ciniki, $args['tnid'], [
            'festival' => $festival,
            'member' => $member,
            'entry' => $entry,
            ]);
