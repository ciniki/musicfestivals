...
function ciniki_musicfestivals_classList($ciniki) {
    ...
    if( isset($args['festival_id']) && $args['festival_id'] != '' ) {
        $strsql .= "AND ciniki_musicfestival_classes.festival_id = '" . ciniki_core_dbQuote($ciniki, $args['festival_id']) . "' ";
    }
    ciniki_core_loadMethod($ciniki, 'ciniki', 'core', 'private', 'dbHashQueryArrayTree');
    $rc = ciniki_core_dbHashQueryArrayTree($ciniki, $strsql, 'ciniki.musicfestivals', array(
        array('container'=>'classes', 'fname'=>'id', 
            'fields'=>array('id', 'festival_id', 'category_id', 'code', 'name', 'permalink', 'sequence', 'flags', 'earlybird_fee', 'fee', 'synopsis')),
        ));
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
