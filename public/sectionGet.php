...
function ciniki_musicfestivals_sectionGet($ciniki) {
    ...
    //
        ...
        if( $section['competitorfees_amount'] != 0 ) {
            $section['competitorfees_amount'] = '$' . number_format($section['competitorfees_amount'], 2);
        } else {
            $section['competitorfees_amount'] = '';
        }
        ...
        if( isset($args['categories']) && $args['categories'] == 'yes' ) {
            ...
            $rc = ciniki_core_dbHashQueryArrayTree($ciniki, $strsql, 'ciniki.musicfestivals', array(
                array('container'=>'categories', 'fname'=>'id', 
                    'fields'=>array('id', 'festival_id', 'section_id', 'groupname', 'name', 'permalink', 'sequence', 'description')),
                ));
            if( $rc['stat'] != 'ok' ) {
                return $rc;
            }
