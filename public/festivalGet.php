...
/*        //
            ...
            if( isset($args['registrations_list']) && $args['registrations_list'] == 'sections' ) {
                $strsql = "SELECT sections.id, "
                    ...
                    . "LEFT JOIN ciniki_musicfestival_registrations AS registrations USE INDEX (festival_id_2) ON ("
                        ...
                        . "AND classes.id = registrations.class_id "
                        . $reg_count_exclude_sql
                        . $ipv_sql
        ...
        if( isset($args['classes']) && $args['classes'] == 'yes' 
            ...
            $strsql = "SELECT classes.id, "
                ...
                . "classes.flags, "
                . "classes.feeflags, "
                . "IF((classes.questionflags&0x01)=0x01, 'yes', '') AS question_provincials, "
                . "IF((classes.questionflags&0x02)=0x02, 'yes', '') AS question_musicfest, "
                . "IF((classes.questionflags&0x04)=0x04, 'yes', '') AS question_canwest, "
                . "classes.titleflags, "
...
//                    . "HAVING ISNULL(timeslot_id) "
            ...
            elseif( isset($args['ssection_id']) && $args['ssection_id'] > 0 ) {
                $strsql = "SELECT divisions.id, "
                    ...
                    . "LEFT JOIN ciniki_musicfestival_adjudicatorrefs AS arefs USE INDEX (adjudicator_id) ON ("
        ...
        if( isset($args['schedule']) && $args['schedule'] == 'adjudicators' ) {
            ...
            $strsql .= "registrations.mark, "
                ...
                . "INNER JOIN ciniki_musicfestival_adjudicatorrefs AS arefs ON ("
                    ...
                    . "AND divisions.id = arefs.object_id "
//                    . "("
//                        . "(arefs.object = 'ciniki.musicfestivals.schedulesection' AND ssections.id = arefs.object_id) "
//                        . "OR (arefs.object = 'ciniki.musicfestivals.scheduledivision' AND divisions.id = arefs.object_id) "
//                        . ")"
                    . "AND arefs.tnid = '" . ciniki_core_dbQuote($ciniki, $args['tnid']) . "' "
                    . ") "
...
//                    'dlists'=>array('classcodes'=>', '),
        ...
        if( isset($args['invoices']) && $args['invoices'] == 'yes' ) {
            ...
            // Get the invoices
            //
            if( isset($args['invoice_typestatus']) ) {
...
//                $strsql .= "DATE_FORMAT(messages.date_added, '%b %e, %Y %l:%i %p') AS date_text ";
        ...
        if( isset($args['members']) && $args['members'] == 'yes' 
            ...
            $strsql = "SELECT members.id, "
                ...
                . "members.flags AS member_options, "
                . "IFNULL(tenants.name, '') AS member_tenant, "
//                . "members.customer_id, "
...
//                . "COUNT(registrations.id) AS num_registrations "
                ...
                . "LEFT JOIN ciniki_tenants AS tenants ON ("
                    . "members.member_tnid = tenants.id "
                    . ") "
...
//                . "GROUP BY members.id "
            ...
            $rc = ciniki_core_dbHashQueryArrayTree($ciniki, $strsql, 'ciniki.musicfestivals', array(
                array('container'=>'members', 'fname'=>'id', 
                    'fields'=>array('id', 'customer_id', 'customer_name', 'name', 'member_tenant', 'shortname', 
                        'category', 'status', 'member_options',
                        'reg_start_dt_display', 'reg_end_dt_display', 'latedays',
                        ),
                    'flags'=>array(
                        'member_options'=>$maps['member']['flags'],
                        ),
                    'utctotz'=>array(
