...
function ciniki_musicfestivals_registrationGet($ciniki) {
    ...
    $rc = ciniki_core_prepareArgs($ciniki, 'no', array(
        ...
        'class'=>array('required'=>'no', 'blank'=>'yes', 'name'=>'Syllabus Class Info'),
        ));
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    //
        ...
        $strsql .= "IFNULL(divisions.name, '') AS division_name, "
            ...
            . "registrations.sr_preferred, "
            . "registrations.sr_conflicts "
        ...
        $rc = ciniki_core_dbHashQueryArrayTree($ciniki, $strsql, 'ciniki.musicfestivals', array(
            array('container'=>'registrations', 'fname'=>'id', 
                'fields'=>array('id', 'festival_id', 'teacher_customer_id', 'teacher2_customer_id',
                    ...
                    'class_id', 
                    'title1', 'composer1', 'opus1', 'movements1', 'musical1', 'arranger1', 'perf_time1', 
                    'title2', 'composer2', 'opus2', 'movements2', 'musical2', 'arranger2', 'perf_time2', 
                    'title3', 'composer3', 'opus3', 'movements3', 'musical3', 'arranger3', 'perf_time3', 
                    'title4', 'composer4', 'opus4', 'movements4', 'musical4', 'arranger4', 'perf_time4', 
                    'title5', 'composer5', 'opus5', 'movements5', 'musical5', 'arranger5', 'perf_time5', 
                    'title6', 'composer6', 'opus6', 'movements6', 'musical6', 'arranger6', 'perf_time6', 
                    'title7', 'composer7', 'opus7', 'movements7', 'musical7', 'arranger7', 'perf_time7', 
        ...
        for($i = 1; $i <= 5; $i++) {
            if( $registration['competitor' . $i . '_id'] > 0 ) {
                ...
                if( $competitor['grade'] != '' ) { $details[] = array('label'=>'Grade', 'value'=>$competitor['grade']); }
...
//                . "AND messages.status > 10 "
    ...
    if( isset($args['class']) && $args['class'] == 'yes' ) {
        ...
        if( $rc['stat'] != 'ok' ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.musicfestivals.1480', 'msg'=>'Unable to load class', 'err'=>$rc['err']));
        }
        if( !isset($rc['class']) ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.musicfestivals.1481', 'msg'=>'Unable to find requested class'));
        }
        $rsp['class'] = $rc['class'];
    }

    if( isset($args['classes']) && $args['classes'] == 'no' ) {
        return $rsp;
    }
...
/*    $strsql = "SELECT id, name "
    ...
    $strsql .= "classes.flags, "
        ...
        . "INNER JOIN ciniki_musicfestival_categories AS categories ON ("
            ...
            . ") "
        . "INNER JOIN ciniki_musicfestival_classes AS classes ON ("
            . "classes.festival_id = '" . ciniki_core_dbQuote($ciniki, $registration['festival_id']) . "' "
