...
// -------
//
function ciniki_musicfestivals_warmupSchedulePDF($ciniki) {
    //
    ...
    $rc = ciniki_musicfestivals_checkAccess($ciniki, $args['tnid'], 'ciniki.musicfestivals.warmupSchedulePDF');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    // Run the template
    //
    ciniki_core_loadMethod($ciniki, 'ciniki', 'musicfestivals', 'templates', 'warmupSchedulePDF');
    $rc = ciniki_musicfestivals_templates_warmupSchedulePDF($ciniki, $args['tnid'], $args);
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
