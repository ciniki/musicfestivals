...
// ---------
// 
function ciniki_musicfestivals_accoladeEmailSend(&$ciniki, $tnid, $args) {
    ...
    $subject = str_replace('{_accoladecriteria_}', $winner['accolade_criteria'], $subject);
    $message = str_replace('{_accoladecriteria_}', $winner['accolade_criteria'], $message);
    ...
    if( isset($args['send']) && $args['send'] == 'all' ) {
        ...
        $sent_emails = [];
        if( isset($args['sent_emails']) && is_array($args['sent_emails']) ) {
            $prev_emails = $args['sent_emails'];
        }
        foreach($emails as $email_address => $email) {
            if( isset($prev_emails) ) {
                if( in_array($email_address, $prev_emails) ) {
                    continue;
                } else {
                    $prev_emails[] = $email_address;
                }
            }
            $sent_emails[] = $email_address;
    ...
    return array('stat'=>'ok', 'sent_emails'=>$sent_emails);
}
?>
