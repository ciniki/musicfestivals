...
function ciniki_musicfestivals_status() {
    ...
    this.menu.open = function(cb) {
        M.api.getJSONCb('ciniki.musicfestivals.sysadminStatus', {'tnid':M.curTenantID}, function(rsp) {
            ...
            p.data = rsp;
            p.sections.festivals.headerValues = ['Festival', 'Status', 'Start', 'End', 'Tenant', 'SMTP', 'Theme', 'Matomo', 'etrans', 'Waiver', 'Stripe', '# Reg'];
            p.sections.festivals.sortTypes = ['text', 'status', 'date', 'date', 'text', 'text', 'text', 'number', 'text', 'text', 'text', 'number'];
            p.sections.festivals.dataMaps = ['festival_name', 'status_text', 'start_date', 'end_date', 'tenant_name', 'smtp', 'theme', 'matomo_id', 'etransfer', 'waiver', 'stripe', 'num_reg'];
