...
function ciniki_musicfestivals_templates_schedulePDF(&$ciniki, $tnid, $args) {
    ...
    if( isset($args['section_adjudicator_bios'])
        ...
        if( $rc['stat'] != 'ok' ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.musicfestivals.976', 'msg'=>'Unable to load adjudicators', 'err'=>$rc['err']));
        }
    ...
    $strsql .= "GROUP_CONCAT(customers.display_name SEPARATOR ', ') AS adjudicator_names, "
...
//    $strsql .= "GROUP BY registrations.id ";
    ...
    if( isset($args['sorting']) && $args['sorting'] == 'location-division' ) {
        ...
        $rc = ciniki_core_dbHashQueryArrayTree($ciniki, $strsql, 'ciniki.musicfestivals', array(
            array('container'=>'sections', 'fname'=>'location_id', 
                'fields'=>array('id'=>'section_id', 'name'=>'location', 'sponsor_settings', 'provincial_settings', 'adjudicator_ids'),
        ...
        $rc = ciniki_core_dbHashQueryArrayTree($ciniki, $strsql, 'ciniki.musicfestivals', array(
            array('container'=>'sections', 'fname'=>'section_id', 
                'fields'=>array('id'=>'section_id', 'name'=>'section_name', 'sponsor_settings', 'provincial_settings', 'adjudicator_ids'),
                'json'=>array('sponsor_settings', 'provincial_settings'),
                ),
            array('container'=>'divisions', 'fname'=>'division_id', 
                'fields'=>array('id'=>'division_id', 'name'=>'division_name', 'date'=>'division_date_text', 
            ...
            if( isset($args['division_header_format']) 
                ...
                foreach($fields as $field) {
                    if( $field == 'namedate' ) {
                        ...
                        if( $division['adjudicator_names'] == '' ) {
                            $division[$field] = $division['location'];
                        } else {
...
//            && $prev_adjudicator_id != $section['adjudicator_id']
            ...
            $adjudicator_ids = explode(',', $section['adjudicator_ids']);
            foreach($adjudicator_ids as $adjudicator_id) {
                if( !isset($adjudicators[$adjudicator_id]) 
                    || $prev_adjudicator_id == $adjudicator_id
                    ) {
                    continue;
                }
                $adjudicator = $adjudicators[$adjudicator_id];
...
/*
            ...
            if( $num_sponsors == 1 ) {
                ...
                if( $rc['stat'] == 'ok' ) {
                    ...
                    // and scaled based on either height or width.
                    if( $available_ratio < $image_ratio ) {
                        $pdf->Image('@'.$img->getImageBlob(), $pdf->left_margin + 130, $y+$h1, $img_width, 0, '', '', 'M', 2, '150', '', false, false,0);
                    } else {
                        $reduced_width = ($img_height/$height)*$width;
                        $pdf->Image('@'.$img->getImageBlob(), $pdf->left_margin + 130 + ((50-$reduced_width)/2), $y+$h1, 0, $img_height, '', '', 'M', 2, '150', '', false, false,0);
                    }
                }
                $pdf->SetY($y);
            } 
...
*/
        ...
        foreach($section['divisions'] as $division) {
            ...
            //
            // FIXME: Need to calculate timeslot heights before printing out divisionheader to know if we need a new page first
            //
...
//                            $row['titles_height'] = $pdf->getStringHeight($row['title_width'], $row['title1']);
                            ...
                            for($i = 1; $i <= 8; $i++ ) {
                                if( isset($reg["fulltitle{$i}"]) && $reg["fulltitle{$i}"] != '' ) {
                                    $row["title{$i}"] = $timeslot['registrations'][$rid]["fulltitle{$i}"];
                        ...
                        } elseif( isset($args['teachers']) && $args['teachers'] == 'yes' && $reg['teacher_name'] != '' ) {
                            $row['dash_width'] = 0; $pdf->getStringWidth('-', '', '') + 3;
                            $row['name_width'] = $pdf->getStringWidth($row['name'], '', '') + 0.25;
                            $row['title_width'] = $pdf->getStringWidth($reg['teacher_name'], '', '') + 0.25;
                            $row['name_width'] = ($w[2]*0.5);
                            $row['title_width'] = $w[2] - $row['name_width'] - $row['dash_width'] - 1;
                            $row['name_height'] = $pdf->getStringHeight(($row['name_width']), $row['name']);
                            $row['height'] = $row['name_height'];
                            $row['title1'] = $reg['teacher_name'];
                            $row['titles_height'] = $pdf->getStringHeight($row['title_width'], $row['title1']);
                            if( $row['titles_height'] > $row['height'] ) {
                                $row['height'] = $row['titles_height'];
                            }
                ...
                if( ciniki_core_checkModuleFlags($ciniki, 'ciniki.musicfestivals', 0x080000) ) {
                    ...
                    if( $description != '' ) {
                        $pdf->SetFont('', '', '12');
                        $pdf->Multicell($w[0], $dh, '', $border, 'R', 0, 0);
                        $pdf->Multicell($w[1], $dh, '', $border, 'R', 0, 0);
                        $pdf->Multicell($w[2], $dh, $description, $border, 'L', 0, 1);
                ...
                foreach($reg_list as $row) {
                    ...
                    } elseif( isset($args['teachers']) && $args['teachers'] == 'yes' ) {
                        $pdf->MultiCell($row['name_width'], $row['height'], $row['name'], 0, 'L', 0, 0);
                        if( isset($row['title1']) && $row['title1'] != '' ) {
                            $pdf->MultiCell($row['title_width'], '', $row['title1'], 0, 'L', 0, 1);
                        } else {
                            $pdf->Ln(5);
        ...
        if( isset($args['provincials_info']) && $args['provincials_info'] == 'yes' 
            ...
            if( $image_id > 0 ) {
                ...
                if( $rc['stat'] == 'ok' ) {
                    ...
                    // and scaled based on either height or width.
                    if( $available_ratio < $image_ratio ) {
                        $pdf->Image('@'.$img->getImageBlob(), $pdf->left_margin+$w[0], $y, $img_width, 0, '', '', 'L', 2, '150');
                    } else {
                        $pdf->Image('@'.$img->getImageBlob(), $pdf->left_margin+$w[0], $y, 0, $h-8, '', '', 'L', 2, '150');
                    }
                }
            }
            $pdf->SetY($y);
