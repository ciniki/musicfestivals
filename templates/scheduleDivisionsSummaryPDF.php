...
// <rsp stat='ok' id='34' />
//
function ciniki_musicfestivals_templates_scheduleDivisionsSummaryPDF(&$ciniki, $tnid, $args) {
    ...
    $strsql = "SELECT ssections.id AS section_id, "
        ...
        . "divisions.division_date AS division_sort_key, "
    ...
    $strsql .= "GROUP BY divisions.id ";
    $strsql .= "ORDER BY ssections.sequence, ssections.name, location_shortname, divisions.division_date, divisions.name, location_shortname ";
    ciniki_core_loadMethod($ciniki, 'ciniki', 'core', 'private', 'dbHashQueryArrayTree');
    $rc = ciniki_core_dbHashQueryArrayTree($ciniki, $strsql, 'ciniki.musicfestivals', array(
        array('container'=>'sections', 'fname'=>'section_id', 
            'fields'=>array('id'=>'section_id', 'name'=>'section_name', 'adjudicator_ids'),
            ),
        array('container'=>'divisions', 'fname'=>'division_id', 
            'fields'=>array('id'=>'division_id', 'name'=>'division_name', 'date'=>'division_date_text', 
                'location_shortname', 'adjudicator_ids', 'adjudicator_names',
   ...
   //                         $this->Image('@'.$this->header_image->getImageBlob(), $this->left_margin, 10, $img_width, $this->header_height-8, '', '', 'L', 2, '150', '', false, false, 0, true);
        ...
        public function TableHeader() {
            ...
            $this->MultiCell($this->cell_widths[2], 0, 'Adjudicator', 1, 'L', 1, 1);
        }

        public function Division($division) {
            $this->SetFont('helvetica', '', 11);
            $lh = $this->getStringHeight($this->cell_widths[0], $division['location_shortname']);
            if( $this->getStringHeight($this->cell_widths[1], $division['name']) > $lh ) {
                $lh = $this->getStringHeight($this->cell_widths[1], $division['name']);
            }
            if( $this->getStringHeight($this->cell_widths[2], $division['adjudicator_names']) > $lh ) {
                $lh = $this->getStringHeight($this->cell_widths[2], $division['adjudicator_names']);
            }
            ...
            $this->SetFont('helvetica', '', 11);
            $this->MultiCell($this->cell_widths[0], $lh, $division['location_shortname'], 1, 'L', 0, 0);
            $this->MultiCell($this->cell_widths[1], $lh, $division['name'], 1, 'L', 0, 0);
            $this->MultiCell($this->cell_widths[2], $lh, $division['adjudicator_names'], 1, 'L', 0, 1);
        }
    }
    ...
    //
    $pdf->header_height = 0;
    $pdf->header_title = 'Schedule Location Adjudicators';
    ...
    $pdf->SetTitle($festival['name'] . ' - Schedule Location Adjudicators');
    $pdf->SetSubject('');
    $pdf->SetKeywords('');
    ...
    foreach($sections as $section) {
        ...
        if( isset($args['schedulesection_id']) ) {  
            $filename = $section['name'] . ' - Divisions Location Adjudicators';
        }
        $pdf->AddPage();
        $pdf->TableHeader();
