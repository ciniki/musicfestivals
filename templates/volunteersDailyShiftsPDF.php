...
// Description
// -----------
// This function generates the list of daily shifts and volunteers
// 
// Arguments
// ---------
// ciniki: 
...
// ---------
// 
function ciniki_musicfestivals_templates_volunteersDailyShiftsPDF(&$ciniki, $tnid, $args) {
    ...
    if( !class_exists('MYPDF') ) {
        class MYPDF extends TCPDF {
            ...
            //public $cell_widths = [40,52,24,24,40];
            ...
            public function setHeaderImage($ciniki, $tnid, $image_id) {
                ...
                if( $rc['stat'] == 'ok' ) {
                    ...
                    if( $rc['stat'] != 'ok' ) {
                        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.musicfestivals.1608', 'msg'=>'No image specified', 'err'=>$rc['err']));
                    }
            ...
            public function Shift($shift) {
                ...
                $lh = $this->getStringHeight($this->cell_widths[0], $shift['location_shortname']);
                ...
                if( $this->getStringHeight($this->cell_widths[4], $shift['names_cell']) > $lh ) {
                    $lh = $this->getStringHeight($this->cell_widths[4], $shift['names_cell']);
                }
                ...
                $this->SetFont('helvetica', '', 11);
                $this->MultiCell($this->cell_widths[0], $lh, $shift['location_shortname'], 1, 'L', 0, 0);
                ...
                $this->MultiCell($this->cell_widths[4], $lh, $shift['names_cell'], 1, 'L', 0, 1);
            }
        }
    }
...
//    $pdf = new MYPDF('P', PDF_UNIT, 'LETTER', true, 'UTF-8', false);
    ...
    foreach($shifts as $shift) {
        ...
        $shift['names_cell'] = preg_replace("/<br>/", "\n", $shift['names_cell']);
        if( $prev_shift_date != $shift['shift_date'] ) {
            $pdf->header_sub_title = 'Volunteers';
            $pdf->header_msg = $shift['shift_date'];
            $pdf->AddPage();
            $pdf->TableHeader();
        }
