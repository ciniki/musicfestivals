...
function ciniki_musicfestivals_templates_accoladesAwardLettersPDF(&$ciniki, $tnid, $args) {
        ...
        class MYPDF extends TCPDF {
            ...
            public function setHeaderImage($ciniki, $tnid, $image_id) {
                ...
                if( $rc['stat'] == 'ok' ) {
                    ...
                    if( $rc['stat'] != 'ok' ) {
                        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.musicfestivals.1546', 'msg'=>'No image specified', 'err'=>$rc['err']));
                    }
    ...
    foreach($args['winners'] as $winner) {
        ...
        $content = str_replace('{_accoladename_}', $winner['accolade_name'], $content);
        if( $winner['awarded_amount'][0] != '$' && $winner['awarded_amount'] > 0 ) {
            $winner['awarded_amount'] = '$' . number_format($winner['awarded_amount'], 0);
        }
        $content = str_replace('{_accoladecriteria_}', $winner['accolade_criteria'], $content);
        $content = str_replace('{_accoladeamount_}', $winner['awarded_amount'], $content);
